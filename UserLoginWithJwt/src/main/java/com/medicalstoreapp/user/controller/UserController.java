...
import com.medicalstoreapp.user.util.DateConverter;
...
public class UserController {
	 ...
	 * @return
	 */
	@DeleteMapping("/orders/byid/{orderId}")
	@PreAuthorize("hasRole('ROLE_PARTICIPANT')")
	 ...
	 */
	@GetMapping("/reports/byreporttype/{type}")
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	 ...
	 */
	@GetMapping("/reports/byperiod/{startdate}/{enddate}")
	@PreAuthorize("hasRole('ROLE_ADMIN')")
    public ReportDetails fetchReportByPeriod( @PathVariable("startdate") String startPeriod,@PathVariable("enddate") String endPeriod) {
        ReportDetails response = userService.findReportByPeriod(startPeriod, endPeriod);
        return response;
    }
